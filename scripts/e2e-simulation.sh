...
prepare_simulation() {
  ...
  FEEDS_CONFIG_FILE="$GIT_REPO_ROOT/config/feeds_config.json"
  SEQUENCER_CONFIG_FILE="$GIT_REPO_ROOT/apps/sequencer/sequencer_config.json"
  DATA_FEEDS_FILE="$GIT_REPO_ROOT/apps/sequencer/src/http_handlers/data_feeds.rs"
...
deploy_contracts() {
  ...
  CONFIG_FILE="$GIT_REPO_ROOT/config/feeds_config.json"
  ...
  yarn hardhat deploy --networks local --config-file "$CONFIG_FILE" || { echo "Deployment failed"; return 1; }
...
run_sequencer_in_background() {
  ...
  cargo run --bin sequencer >"$SEQENCER_LOG_FILE" 2>&1 &
...
cleanup() {
  ...
  git checkout "$DEPLOYMENT_FILE" "$FEEDS_CONFIG_FILE" "$SEQUENCER_CONFIG_FILE" "$DATA_FEEDS_FILE"
}
