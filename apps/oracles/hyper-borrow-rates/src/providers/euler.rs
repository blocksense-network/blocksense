...
async fn fetch_borrow_rate(
    ...
    let calldata = instance.getAPYs(vault_address).calldata().clone();
...
pub async fn fetch_borrow_rates_from_euler(
    feeds_config: Option<&Vec<FeedConfig>>,
) -> Result<RatesPerFeed> {
...
async fn fetch_borrow_rates_on_ethereum_mainnet(feeds: &Vec<FeedConfig>) -> Result<RatesPerFeed> {
    let mut borrow_rates: RatesPerFeed = RatesPerFeed::new();
...
impl FromStr for SupportedNetworks {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "ethereum-mainnet" => Ok(SupportedNetworks::EthereumMainnet),
