...
pub async fn fetch_borrow_rates_from_morpho(
    ...
    println!("Final borrow rates: {:?}", borrow_rates);
    Ok(borrow_rates)
}
...
async fn fetch_borrow_rates_for_network(
    ...
    for feed in feeds {
        let (market_id_str, morpho_core_address) = match &feed.arguments {
            Marketplace::Morpho(args) => (&args.market_id, args.morpho_core_address),
            _ => continue,
        };

        // Convert hex string to B256
        let market_id_b256 = match B256::from_str(market_id_str) {
            Ok(id) => id,
            Err(err) => {
                warn!(
                    "Invalid market_id '{}' for feed {}: {}",
                    market_id_str, feed.feed_id, err
                );
                continue;
            }
        };
            ...
            Ok(rate) => {
                borrow_rates.insert(
                    ...
                    BorrowRateInfo {
                        ...
                        borrow_rate: apr_from_per_sec_wad(rate),
                    },
                );
            }
            Err(err) => {
