...
async fn fetch_all(_resourses: &[FeedConfig]) -> Result<ResponseEthCall> {
    ...
    let request_body =
        serde_json::to_string(&eth_call).expect("Serialization failed, this is impossible");

    let mut req = Request::builder();
    req.method(Method::Post);
    req.header("Content-Type", "application/json");
    req.uri(rpc_url.to_string());
    ...
    let value: ResponseEthCall = serde_json::from_str(&string)?;
    Ok(value)
}
...
fn get_resources_from_settings(settings: &Settings) -> Result<Vec<FeedConfig>> {
    ...
    for feed_setting in &settings.data_feeds {
        ...
        feed_config.feed_id = feed_setting.id.parse::<u64>().unwrap();
