...
describe('`data-providers.ts` tests', () => {
  ...
  test('should return an array with one outliner', () => {
    const input: Record<string, Record<string, number>[]> = {
      'USDS / USDT': [{ Binance: 0 }, { Bitget: 1.0006 }],
    };
    const expectedResult: string[] = ['Binance'];
    const result = detectOutliers(input);
    expect(result).toEqual(expectedResult);
  ...
  test('should return an empty array when multiple assets have no outliers', () => {
    const input: Record<string, Record<string, number>[]> = {
      'BTC / USDT': [{ Binance: 30000 }, { Bitget: 30010 }, { Bybit: 29990 }],
      'ETH / USDT': [{ Binance: 2000 }, { Bitget: 2005 }, { KuCoin: 1995 }],
    };
    const expectedResult: string[] = [];
    const result = detectOutliers(input);
    expect(result).toEqual(expectedResult);
  ...
  test('should return an empty array when all exchanges have a price of 0', () => {
    const input: Record<string, Record<string, number>[]> = {
