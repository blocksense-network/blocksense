...
describe.sequential('E2E Tests with process-compose', () => {
  ...
  it.effect('Test sequencer config is available and in correct format', () =>
    Effect.gen(function* () {
      ...
      const sequencerConfig = yield* sequencer.getConfig(sequencerConfigUrl);
      ...
      Effect.tap(config =>
        Effect.gen(function* () {
          ...
          feedsConfig = yield* Effect.tryPromise(() =>
            fetchAndDecodeJSON(NewFeedsConfigSchema, sequencerFeedsConfigUrl),
          );
  ...
  describe.sequential('Reporter behavior based on logs', () => {
    ...
    it.effect('Reporter should NOT panic', () =>
      Effect.gen(function* () {
        const logChecker = yield* LogChecker;
        const result = yield* logChecker.assertDoesNotContain({
      ...
      }).pipe(Effect.provide(LogCheckerLive)),
    );
