...
impl BuildConfig {
    ...
    async fn read_secrets(config: &mut BlocksenseConfig) -> Result<()> {
        replace_filepath_with_secret_key(&mut config.reporter_info.secret_key).await?;
        tracing::info!("Secret key {:?}", &config.reporter_info.secret_key);

        replace_filepath_with_secret_key(&mut config.reporter_info.second_consensus_secret_key)
            .await?;
        tracing::info!(
            "Second consensus secret key {:?}",
            &config.reporter_info.second_consensus_secret_key
        );
...
async fn replace_filepath_with_secret_key(filepath: &mut String) -> Result<()> {
    let secret_key = fs::read_to_string(&filepath)
        .await
        .context(format!("No such file - {}", &filepath))?;
    *filepath = secret_key.trim().to_string();
    Ok(())
}
