...
impl FeedSlotsProcessor {
        ...
        for kv in reports {
            match kv.1 {
                FeedResult::Result { result } => {
                    match &result {
                        FeedType::Numerical(_) => values.push(result.clone()),
                        FeedType::Text(_) => {
            ...
            match kv.1 {
                FeedResult::Result { result } => {
                    match &result {
                        ...
                        FeedType::Text(_) => values.push(result.clone()),
                    };
                }
                FeedResult::Error { .. } => {
                    warn!(
    ...
    pub async fn check_for_diviation_criteria(
        &self,
        feed: &Arc<RwLock<FeedMetaData>>,
        sequencer_state: &Data<SequencerState>,
        history: &Arc<RwLock<FeedAggregateHistory>>,
    ) -> bool {
        let (is_oneshot, quorum_percentage, min_deviation_percentage) = {
            let datafeed = feed.read().await;
            (
                datafeed.is_oneshot(),
                datafeed.get_quorum_percentage(),
                datafeed.get_min_deviation_percentage() as f64,
            )
        };
        if is_oneshot {
            return false;
        ...
        let num_valid_reporters = self
            .get_num_valid_reportes(&sequencer_state.reporters)
            .await as f32;
        let required_votes_count = quorum_percentage * num_valid_reporters;
        let last_value = {
            let history_guard = history.read().await;
            match history_guard.last(feed_id) {
                Some(FeedType::Numerical(value)) => *value,
                _ => {
                    return false;
                }
            }
        ...
        let feed_type = self.get_feed_type(feed, feed_id).await;
        match feed_type.as_str() {
            "Numerical" => {
                let current_time_as_ms = current_unix_time();
                let slot = self.get_slot(feed, current_time_as_ms).await;
                let feed_id = self.key;
                let reports: Arc<RwLock<feed_registry::registry::FeedReports>> = match self
                ...
                debug!("Get a read lock on reports [feed {feed_id}]");
                let reports = reports.read().await;
                debug!("Acquired a read lock on reports [feed {feed_id}]");
                let values = self.process_numerical_reports(&reports.report, slot);
                let res = if (values.len() as f32) < required_votes_count {
                    false
                } else {
                    match self.aggregate_values(feed, values).await {
                        FeedType::Numerical(new_value) => {
                            new_value < last_value * (1.0f64 - min_deviation_percentage)
                                || new_value > last_value * (1.0f64 + min_deviation_percentage)
                        }
                        _ => false,
                    }
    ...
    async fn get_slot(&self, feed: &Arc<RwLock<FeedMetaData>>, current_time_as_ms: u128) -> u64 {
        let feed_id = self.key;
    ...
    async fn get_feed_type(&self, feed: &Arc<RwLock<FeedMetaData>>, feed_id: u32) -> String {
        debug!("Get a read lock on reports [feed {feed_id}]");
    ...
    async fn consume_reports(
        ...
        let values: Vec<FeedType> = match feed_type.as_str() {
            "Numerical" => self.process_numerical_reports(reports, slot),
            "Text" => self.process_text_reports(reports, slot),
            _ => vec![],
        };

        if !values.is_empty() {
            let total_votes_count = values.len() as f32;
            let num_valid_reporters = self.get_num_valid_reportes(reporters).await;
            ...
            self.increase_feed_metrics(feed_metrics, is_quorum_reached)
                .await;

            // Dispatch to concrete FeedAggregate implementation.
            let result_post_to_contract = self.aggregate_values(&feed, values).await;
                ...
                .await
            {
                Some(reports) => {
                    ...
                    match feed_type.as_str() {
                        ...
                        "Text" => {}
                        _ => {
                            error!("Insupported feed type {feed_type} for feed: {}", self.name);
                            return;
                        }
                    };
        ...
        if let Some(result_post_to_contract) = consumed_reports.result_post_to_contract {
            if consumed_reports.is_quorum_reached {
                if consumed_reports.skip_publishing {
                    info!("Skipping publishing for feed_id = {} change is lower then threshold of {} %", feed_id, skip_publish_if_less_then_percentage * 100.0f64)
                } else {
    ...
    async fn perform_anomaly_detection(
        ...
        if numerical_vec.len() > AD_MIN_DATA_POINTS_THRESHOLD {
            ...
            Err(eyre!(
                "Not enough values for anomaly detection, num_values {} threshold: {}",
                numerical_vec.len(),
    ...
    async fn increase_feed_metrics(
        &self,
        feed_metrics: Option<Arc<RwLock<FeedsMetrics>>>,
    ...
    pub async fn peridodic_check_for_diviation(
        &self,
        feed: &Arc<RwLock<FeedMetaData>>,
        sequencer_state: &Data<SequencerState>,
        history: &Arc<RwLock<FeedAggregateHistory>>,
    ) {
        let sleep_interval_in_ms = feed.read().await.get_peridic_diviation_check_ms();
        loop {
            let time_to_await: Duration = Duration::from_millis(sleep_interval_in_ms);
            let mut interval = time::interval(time_to_await);
            interval.tick().await; // The first tick completes immediately.
            interval.tick().await;
            if self
                .check_for_diviation_criteria(feed, sequencer_state, history)
                .await
            {
                info!("Triggering diviation report for feed_id = {}", self.key);
                return;
            } else {
                debug!(
                    "Check for diviation publishing for feed_id = {} returned false",
                    self.key
                );
            }
        }
    ...
    pub async fn start_loop(
        ...
        let (is_oneshot, report_interval_ms, first_report_start_time, peridic_diviation_check_ms) = {
            let datafeed = feed.read().await;
            (
                datafeed.is_oneshot(),
                datafeed.get_report_interval_ms(),
                datafeed.get_first_report_start_time_ms(),
                datafeed.get_peridic_diviation_check_ms(),
            )
        };
        ...
        loop {
            ...
            if peridic_diviation_check_ms > 0 {
                ...
                tokio::select! {
                    ...
                    _ = self.peridodic_check_for_diviation(feed, sequencer_state, history) => {
                        is_processed = true;
                        self.process_end_of_slot(
                            feed.clone(),
                            sequencer_state,
                            history,
                            feed_metrics.clone(),
...
mod tests {
    ...
    const MIN_DEVIATION_PERCENTAGE: f32 = 0.0f32;
    ...
    async fn test_feed_slots_processor_loop() {
        ...
        let min_deviation_percentage = MIN_DEVIATION_PERCENTAGE;
    ...
    #[tokio::test]
    async fn test_feed_slots_processor_loop_diviation_report_based_on_threshold() {
        // setup
        let name = "test_feed_slots_processor_loop_diviation_report_based_on_threshold";
        ...
        let (sequencer_state, mut rx) = create_sequencer_state_from_sequencer_config(
            cfg,
            "test_feed_slots_processor_loop_diviation_report_based_on_threshold",
        )
        .await;
