...
use alloy::hex;
use blockchain_data_model::{AddRemoveFeeds, BlockFeedConfig, BlockHeader, Resources};
        ...
        if let Ok(bytes) = hex::decode(header) {
            ...
            let process_block = header.issuer_id != sequencer_id
                || sequencer_state
                    ...
                    < header.block_height;
            if process_block {
                if let Some(add_remove_feeds) = block["AddRemoveFeeds"].as_str() {
                    if let Ok(bytes) = hex::decode(add_remove_feeds) {
                        let add_remove_feeds = AddRemoveFeeds::deserialize(&bytes);
