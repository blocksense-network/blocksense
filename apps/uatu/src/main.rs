...
fn fetch_and_store_match_data() -> Result<(), Box<dyn Error>> {
            ...
            if let Some(preview_link) = row.select(&preview_link_selector).next() {
                if let Some(href) = preview_link.value().attr("href") {
                    if let Some(match_id) = href.split('/').last() {
                        ...
                        if let (Some(home_team_img), Some(away_team_img)) = (home_team_td, away_team_td) {
                            ...
                            let match_time = preview_link.select(&time_selector)
                                .next()
                            ...
                            println!(
                                "Date: {}, Match ID: {}, Time: {}, Home team ID: {}, Away team ID: {}",
                                last_known_date, match_id, match_time, home_team_id, away_team_id
                            );
                        }
                    }
                }
    ...
    // Database URL
    let database_url = "mysql://user:password@localhost:3306/schemas";
    ...
    for match_info in matches {
        conn.exec_drop(
            r"INSERT IGNORE INTO football_events (markt_id, team_a_markt_id, team_b_markt_id, status, event_datetime, description, schema_id)
              VALUES (:match_id, :home_team_id, :away_team_id, 'draft', :event_datetime, '', 1)",
            params! {
