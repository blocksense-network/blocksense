...
impl RpcProvider {
    ...
    pub fn get_contract_address(&self, name: &str) -> Result<Address> {
        let address = self
            .get_contract(name)
            .ok_or(anyhow!("{name} contract is not set!"))?
            .address
            .ok_or(anyhow!("{name} contract address is not set!"))?;
        Ok(address)
    }
    ...
    pub async fn get_latest_values(
        &self,
        feed_ids: &[FeedId],
    ) -> Result<Vec<Result<PublishedFeedUpdate, PublishedFeedUpdateError>>, anyhow::Error> {
    ...
    pub async fn deploy_contract(&mut self, contract_name: &str) -> Result<String> {
        ...
        let mut bytecode = self
            ...
            .ok_or(anyhow!("{contract_name} contract is not set!"))?
            .byte_code
            .ok_or(anyhow!(
    ...
    pub async fn load_history_from_chain(
        ...
        if !latest.is_empty() {
            ...
            let Some(capacity) = self.get_history_capacity(feed_id) else {
                return Err(anyhow!("History is not registered for feed id = {feed_id}"));
            };
