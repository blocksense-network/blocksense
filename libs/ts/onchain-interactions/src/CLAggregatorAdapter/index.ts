...
export class CLAggregatorAdapter {
  ...
  async getCLAggregatorAdapterData(): Promise<CLAggregatorAdapterData> {
    ...
    return {
      dataFeedStore: result[0].result as `0x${string}`,
      decimals: result[1].result as number,
      description: result[2].result as string,
      id: result[3].result as bigint,
      latestAnswer: result[4].result as bigint,
      latestRound: result[5].result as bigint,
      latestRoundData: parseRoundData(
        result[6].result as readonly [bigint, bigint, bigint, bigint, bigint],
      ),
    };
  }
}
