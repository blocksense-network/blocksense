...
import { ADFSGenericWrapper, ADFSReadACWrapper } from './utils/wrappers';
import {
...
describe('AggregatedDataFeedStoreReadAC', () => {
  let contract: ADFSReadACWrapper;
  let signers: HardhatEthersSigner[];
  let accessControlOwner: HardhatEthersSigner;
  let readAccessControlOwner: HardhatEthersSigner;
  ...
  beforeEach(async () => {
    ...
    readAccessControlOwner = signers[2];

    contract = new ADFSReadACWrapper();
    await contract.init(accessControlOwner, readAccessControlOwner);
    ...
    await contract.readAccessControl.setAdminStates(
      readAccessControlOwner,
      [sequencer.address],
      [true],
    );
  });
  ...
  it('Should revert if caller is not in read access control', async () => {
    await contract.setFeeds(sequencer, feeds);

    await expect(
      contract.getValues(signers[10], feeds, {
        operations: feeds.map(() => ReadOp.GetLatestSingleFeed),
      }),
    ).to.be.reverted;
  });
