...
describe('ADFS input parser', () => {
  ...
  it('Should decode feeds correctly when rb index is wrong', async () => {
    ...
    const parsedData = decodeADFSCalldata(input, true, false);
    ...
    const feedsNotFound = feedIds.filter(
      feedId => !parsedData.feeds.find(f => f.feedId === BigInt(feedId)),
    );
    expect(feedsNotFound.length).to.equal(
      0,
      `Feeds not found: ${feedsNotFound}`,
    );

    const feedsNotParsed = parsedData.feeds.filter(
      feed => !feedIds.find(f => BigInt(f) === feed.feedId),
    );
    expect(feedsNotParsed.length).to.equal(
      0,
      `Feeds not parsed: ${feedsNotParsed}`,
    );

    expect(() => decodeADFSCalldata(input)).to.throw(
      'invalid ring buffer index 8305 for feedId 99',
    );
  });
