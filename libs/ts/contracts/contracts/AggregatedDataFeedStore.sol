        ...
        for {
          ...
          sstore(
            ...
            and(
              ...
              // Each storage slot packs 16 ring buffer indices (one per feedId in the group),
              // each index encoded in 16 bits. Valid ring buffer indices are in [0, 8192)
              // (13 bits). Writers are expected to wrap indices modulo 8192, but we defensively
              ...
              // Operating on whole 32-byte words keeps this check cheap while preventing
              // subtle corruption / leakage across feeds.
