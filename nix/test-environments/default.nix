        ...
        blocksense = pkgs.testers.runNixOSTest {
          ...
          testScript = ''
            ...
            while machine.execute("curl http://127.0.0.1:5553/get_history | jq '.aggregate_history | all(. == [])'")[1].strip() == "true":
              print(machine.execute("curl http://127.0.0.1:5553/get_history | jq '.aggregate_history | all(. == [])'"));
              print(machine.execute("curl http://127.0.0.1:5553/get_history | jq '.aggregate_history | all(. == [])'")[1]);
              print(machine.execute("curl http://127.0.0.1:5553/get_history | jq '.aggregate_history | all(. == [])'")[1].strip());
              print(machine.execute("curl http://127.0.0.1:5553/get_history | jq '.aggregate_history | all(. == [])'")[1].strip() == "true");
              machine.execute("sleep 10");
              runs += 1;
              if runs > 60:
